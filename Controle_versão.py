    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if version_element is not None:
                ...
                # Update 'Data_Atualização_Atual' with the current date and time
                # df.loc[index, 'Data_Atualização_Atual'] = now.strftime("%d/%m/%Y %H:%M:%S")
                ...
                if row[1] == version:
                    ...
                    df.loc[index, 'Data_Atualização_Anterior'] = df.loc[index, 'Data_Atualização_Atual']
                    df.loc[index, 'Data_Atualização_Atual'] = now.strftime("%d/%m/%Y %H:%M:%S")
