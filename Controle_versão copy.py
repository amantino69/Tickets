...
# Convert 'Data_atualização _Anterior' columns to string
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if version_element is not None:
                ...
                if row['Versão_Anterior'] != row['Versão_Atual']:
                    # Update 'Versão_Anterior' with the value of 'Versão_Atual'
                    df.loc[index, 'Versão_Anterior'] = row['Versão_Atual']
                    # Keep the 'Data_atualização _Anterior' date
                else:
                    # Update 'Data_atualização _Anterior' with the current date and time
